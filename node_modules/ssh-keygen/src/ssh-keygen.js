...
function checkAvailability(location, force, callback){
	...
	fs.exists(location, function(keyExists){
		...
		fs.exists(pubLocation, function(sshPublicKeyExists){
			doForce(keyExists, sshPublicKeyExists);
		})
	});
	function doForce(keyExists, sshPublicKeyExists){
		if(!force && keyExists) return callback(location+' already exists');
		if(!force && sshPublicKeyExists) return callback(pubLocation+' already exists');
		if(!keyExists && !sshPublicKeyExists) return callback();
		if(keyExists){
			log('removing '+location);
			fs.unlink(location, function(err){
				if(err) return callback(err);
				keyExists = false;
				if(!keyExists && !sshPublicKeyExists) callback();
			});
		}
		...
		if(read){
			...
			fs.readFile(location, 'utf8', function(err, key){
				...
				function readPubKey(){
					...
					fs.readFile(pubLocation, 'utf8', function(err, sshPublicKey){
						if(destroy){
							...
							fs.unlink(pubLocation, function(err){
								...
								sshPublicKey = sshPublicKey.toString();
								sshPublicKey = sshPublicKey.substring(0, sshPublicKey.lastIndexOf("\n")).trim();
								return callback(undefined, {
									key: key, sshPublicKey: sshPublicKey
								});
							});
						} else callback(undefined, { key: key, sshPublicKey: sshPublicKey });
					});
				}
			});
